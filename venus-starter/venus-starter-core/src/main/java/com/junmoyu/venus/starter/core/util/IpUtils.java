...
public final class IpUtils {
    ...
    public static String getHost(final String filterHost) {
        ...
        // filter matching ip
        if (ASTERISK_SEPARATOR.equals(filterHost) || "".equals(filterHost)) {
            pattern = null;
        } else if (filterHost != null && !filterHost.contains(ASTERISK_SEPARATOR) && !isCompleteHost(filterHost)) {
            pattern = filterHost + ASTERISK_SEPARATOR;
        }
                ...
                while (addresses.hasMoreElements()) {
                    ...
                    if (inetAddress != null && !inetAddress.isLoopbackAddress()) {
                        if (inetAddress instanceof Inet4Address && isCompleteHost(inetAddress.getHostAddress())) {
                            netCard = new NetCard(inetAddress.getHostAddress(), getName(networkInterface.getName()), getNamePostfix(networkInterface.getName()), Integer.parseInt(inetAddress.getHostAddress().split("\\.")[3]));
                            ipv4Result.add(netCard);
                        } else {
                            netCard = new NetCard(inetAddress.getHostAddress(), getName(networkInterface.getName()), getNamePostfix(networkInterface.getName()));
